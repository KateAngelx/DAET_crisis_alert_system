...
import {
  AlertTriangle, Search, Filter, Eye, ChevronLeft, ChevronRight,
...
export default function AdminIncidentsPage() {
  ...
  const { incidents, fetchIncidents, updateIncidentStatus, getFilteredIncidents, getDashboardStats, setFilters, filters, loading } = useIncidentStore();
  ...
  return (
    <AuthGuard requiredRole="admin">
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-1">Incident Reports</h1>
          <p className="text-gray-600">Monitor and manage all user-submitted crisis reports.</p>
        </div>
        ...
        {loading ? (
          <p className="text-gray-400 animate-pulse font-bold uppercase text-xs">Loading incidents...</p>
        ) : paginated.length === 0 ? (
          <Card className="p-12 text-center">
            <FileText size={48} className="mx-auto text-gray-200 mb-4" />
            <p className="text-gray-400 font-bold uppercase text-xs">No incident reports found</p>
          </Card>
        ) : (
